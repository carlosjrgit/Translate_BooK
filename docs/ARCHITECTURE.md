...
- Camada responsável pela persistência, integridade e governança de memórias editoriais de longo prazo:
  ...
  - **`StyleBible`**: Governa as convenções e estilo editorial da obra em 10 dimensões canônicas: narrador, pessoa narrativa, tempo verbal predominante, nível de formalidade, padrão de diálogo, tratamento de palavrões, formas de tratamento, pontuação editorial, tratamento de títulos e convenções internas. Toda regra possui snippets de evidência textual de origem (`StyleEvidence`), pontuação de confiança, suporte a regras travadas (`locked=True`) imutáveis e validadores de QA (`validate_text`, `validate_dialogue_style`, etc.).
  - **`StoryMemory`**: Mapeia a continuidade narrativa da obra sem depender de prompts massivos: resumos por capítulo (`ChapterSummary`), estados vitais e dinâmicos de personagens (`CharacterState`), laços relacionais (`StoryRelationship`), eventos cronológicos (`StoryEvent`), fatos persistentes do universo (`PersistentFact`) e referências cruzadas (`StoryCrossReference`). Todos os registros distinguem categoricamente fatos explícitos de inferências (`is_inferred: bool`, `source_type`).
  - **Detecção de Contradições**: Algoritmos dedicados (`detect_contradictions`) identificam quebras de continuidade na história (ex: personagem morto em ação posterior sem justificativa, fatos conflitantes) e conflitos entre novas descobertas analíticas e regras de estilo travadas.
  - **`MemoryManager`**: Fachada unificada que coordena Character Memory, Glossary, TM, Style Bible, Story Memory e a camada relacional SQLite (tabelas `story_summaries`, `character_states`, `story_relationships`, `story_events`, `story_facts`, `story_cross_references` e `style_bible`). Implementa auditoria de conflitos ponta a ponta (`detect_all_conflicts`) cobrindo divergências terminológicas, violações de termos travados e anomalias narrativas.
  ...
  - **Governança e Travamento (`locked`)**: Termos, segmentos e regras com `locked=True` não podem ser alterados ou sobrescritos sem autorização explícita (`force=True`).
...
### 3.5 `context`
- **`ContextRetrieval` & `StoryContextSnapshot`**: 
  - MADLAD-400 opera como modelo seq2seq neural focado em tradução e não como LLM de janela infinita. A camada de contexto não constrói mega-prompts disfuncionais.
  - O método `StoryMemory.get_scene_context_snapshot` gera um snapshot enxuto e estritamente delimitado (`max_facts`, `max_events`), priorizando fatos explícitos sobre inferências e selecionando exclusivamente os estados vitais dos personagens presentes na cena atual.
  - Empacota o contexto ótimo para o segmento atual (parágrafos limítrofes, resumo local, personagens presentes na cena e termos aplicáveis do glossário), evitando tanto subcontexto quanto sobrecarga do modelo.
